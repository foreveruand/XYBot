...
class Video(BaseModel):
    ...
    @classmethod
    async def from_id(cls, bili_number: str, options: str | None = None):
        logger.info(f"Parsing video {bili_number}")
        video_info = None
        for i in range(BILICHAT_NETERROR_RETRY):
            try:
                ...
                logger.warning(f"请求超时，重试第 {i + 1}/{BILICHAT_NETERROR_RETRY} 次")
        else:
    ...
    async def get_image(self, style: str):
        ...
        for i in range(BILICHAT_NETERROR_RETRY):
            try:
                ...
                e = exc
                logger.error(
                    f"绘制视频封面 av{self.id} 请求超时: {e}, 重试第 {i + 1}/{BILICHAT_NETERROR_RETRY} 次"
                )
